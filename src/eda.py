...
scaled = scaler.fit_transform(grouped_copy)

similarity = euclidean_distances(grouped,grouped) #cluster 5 & 7 have largest ED

c1 = 5
c2 = 7
cluster1_means = scaled[c1,:]
cluster2_means = scaled[c2,:]
...
ax.set_title('Normalized Feature Averages For Two Distant Clusters')
ax.legend((p1[0], p2[0]), ('Cluster 6', 'Cluster 8'))
...
ax.set_xlabel('Audio Features',fontsize=12)
ax.set_ylabel('Scaled Value Range',fontsize=12)
ax.set_title("Feature Value Range of Scaled Audio Features For All Songs")
